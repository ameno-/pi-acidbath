...
---
> Shipped with Acidbath. Drive a live Hunk session via `hunk session *`. Do not run interactive `hunk diff` / `hunk show` from the agent. Canonical copy also lives in `ameno-/lib` as `hunk-review`.

> Vendored from `hunkdiff` (`hunk skill path`). Refresh after upgrading hunkdiff. Do not run interactive `hunk diff` / `hunk show` from an agent; drive a live session with `hunk session *`. Git pager is `hunk pager`.
